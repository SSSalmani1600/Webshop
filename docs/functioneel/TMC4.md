# **Website design sprint 4**

[Figma bestand](https://www.figma.com/design/g39qocUOyfUyU52aV4k7I0/Real-Estate-Business-Website-UI-Template---Dark-Theme-%7C-Produce-UI--Community-?node-id=45-2&p=f&t=eWnAM7ROP2yEdwBB-0)


# **Gebruikerstest sprint 4**

| **Testpersoon:** 2 testers (medestudenten) 
| **Tijdsduur:** 10 minuten per testpersoon 
| **Datum:** 03-05-2025 |

---
## **Wat is getest en hoe?**  

Ik heb heb design van de product overview page laten testen en visuele feedback gevraagd op de live website. Hieronder volgt een overzicht van de geteste onderdelen

| **Wat is getest?**  |
|-------------------------|
| **Product overview**             |

---

## **1. Feedback**  
Tijdens de gebruikerstest zijn de volgende verbeterpunten geconstateerd:  

### **Visuele duidelijkheid en interactie**  
- **Zoekfunctie**: Misschien is het een idee om een zoekfunctie toe te voegen, zodat het voor de gebruiker makkelijker wordt. 
- **Taal**: Op sommige plekken wordt Engels en Nederlands door elkaar gehaald, kies er 1.  
- **Prijs**: De prijs wordt getoond in Dollars niet in Euro's.  

### **Kleuren en styling**  
- **Kleur consistentie**: De pagina heeft niet echt een consistente kleur combinatie.   

De gebruikers hebben besproken dat de styling van bepaalde componenten niet goed aansloot bij de visuele stijl van de de website.

---

## **2. Actiepunten**  

### **Visuele duidelijkheid en interactie**  
- **Zoekfunctie**: Ik ga een zoekfunctie toevoegen in het design. 
- **Taal**: Ik ga met de PO overleggen welke taal hij op de website wil en alles naar die taal veranderen. 
- **Prijs**: Ik ga de prijs op de website veranderen naar euro's.

### **Kleuren en styling**  
- **Kleur consistentie**: Ik ga de kleur van enkele componenten veranderen naar de huis style van de website, zodat het allemaal een mooi geheel wordt.

---

## **3. Conclusie**  
De gebruikerstest heeft aangetoond dat er visuele inconsistenties zijn die de gebruikers ervaring minder vloeiend maken. Er zijn opmerkingen gemaakt over de styling van bepaalde componenten, die niet goed aansluiten bij de rest van de website.  

Door deze verbeterpunten door te voeren, zal de website een meer samenhangende en professionele uitstraling krijgen. Met name de visuele aanpassingen en het verfijnen van de styling zullen bijdragen aan een betere gebruikerservaring.  

Om de effectiviteit van deze verbeteringen te testen, wordt een hertest gepland nadat de wijzigingen zijn doorgevoerd.  

